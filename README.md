...
一个可以在桌面上互动的虚拟形象桌宠 Android 应用：**Live2D 骨骼动画**的蓝发少女「灵汐」会显示在所有应用和桌面之上，60fps 丝滑流畅，单击互动、双击摸头、拖拽移动，还能接入 AI 陪你聊天。
...
| 🎨 Live2D 动画 | 内置 Haru / 白兮 Baixi / NOIR 三款模型可切换，骨骼动画 60fps；支持自定义 model3.json URL |
...
默认使用 **Live2D 骨骼动画**，内置三款模型：**Haru**（Live2D 官方示例·蓝发少女）、**白兮 Baixi** 与 **NOIR**（均为 VTS 模型），可在设置中切换，也支持自定义 model3.json URL，并保留社区二创的 **DeepSeek娘（蓝发鲸鱼女仆）** 精灵图作为「经典像素」风格，来自开源同人项目 [xpy12367/codex-pet-DeepSeek-girl](https://github.com/xpy12367/codex-pet-DeepSeek-girl)（1536×2288，8×11 精灵图，9 组动画 + 16 方向注视）。
...
`dist/lingxi-v1.6.apk` 是已签名可直接安装的 APK（侧载安装，开启「允许安装未知来源」）。
