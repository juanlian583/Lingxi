...
"$APKSIGNER" sign --ks "$KEYSTORE" --ks-key-alias lingxi \
    ...
    --out dist/lingxi-v1.6.7.apk build/withdex.apk

echo "[8/8] 验证"
"$APKSIGNER" verify --verbose dist/lingxi-v1.6.7.apk
echo "---- badging ----"
run_aapt2 dump badging dist/lingxi-v1.6.7.apk | grep -E "^(package|application-label|sdkVersion|targetSdkVersion|uses-permission)" | head -12
echo "---- APK 内容 ----"
python3 - <<'EOF'
import zipfile
z = zipfile.ZipFile("dist/lingxi-v1.6.7.apk")
for n in z.namelist():
    print("  ", n)
print("APK size:", __import__('os').path.getsize("dist/lingxi-v1.6.7.apk"), "bytes")
EOF
echo "✅ 构建完成: dist/lingxi-v1.6.7.apk"
